...
directions = {
    "msk": {'city': 'Москвы', 'href': '/departure/msk'},
    "spb": {'city': 'Петербурга', 'href': '/departure/spb'},
    "nsk": {'city': 'Новосибирска', 'href': '/departure/nsk'},
    "ekb": {'city': 'Екатеринбурга', 'href': '/departure/ekb'},
    "kazan": {'city': 'Казани', 'href': '/departure/kazan'}
}
min_cost = 100000


def main_view(request):
    random_tours, random_base = set(), []
    while len(random_tours) != 6:
        random_tours.add(randint(1, 16))
    for i in random_tours:
        random_base += [{i: tours[i]}]
...
def departure_view(request, departure):
    try:
        city = directions[departure]['city']
    except KeyError:
        raise Http404
    tours_from_city = []
    min_cost, max_cost = 100000, 1
    min_nights, max_nights = 100, 0
    for key, value in tours.items():
        if departure == value['departure']:
            tours_from_city += [{key: value}]
            min_cost = min(min_cost, value['price'])
            max_cost = max(max_cost, value['price'])
            min_nights = min(min_nights, value['nights'])
            max_nights = max(max_nights, value['nights'])
    print(len(tours_from_city))
    return render(request, 'tours/departure.html', context={
        'city': city,
        'departures': directions,
        'request.path': request.path,
        'tours_from_city': tours_from_city,
        'count': len(tours_from_city),
        'min_cost': min_cost,
        'max_cost': max_cost,
...
def tour_view(request, pk):
    ...
    return render(request, 'tours/tour.html', context={
        'hotel': tours[pk],
        'stars': int(tours[pk]['stars'])*'*',
        'from_city': directions[tours[pk]['departure']],
        'departures': directions
    })
